...
# 1. FIX: Read from TTY for curl | bash compatibility
...
# 2. FIX: Fetch remote assets before using them
if [ ! -f "$HOME/.ssh/id_rsa" ]; then
    echo "Fetching encrypted SSH keys..."
    curl -fsSL "$BASE_URL/ssh.tar.gpg" -o /tmp/ssh.tar.gpg
    gpg --decrypt /tmp/ssh.tar.gpg | tar xf - -C ~
...
# 3. FIX: Use HTTPS for initial clone (in case SSH isn't ready yet)
if [ ! -d "$HOME/.files" ]; then
    git clone --recursive "$REPO_URL" "$HOME/.files"
fi
cd "$HOME/.files"
...
if has rclone; then
    # Fetch rclone.txt if it doesn't exist locally
    if [ ! -f "rclone.txt" ]; then
        curl -fsSL "$BASE_URL/rclone.txt" -o rclone.txt
    fi
